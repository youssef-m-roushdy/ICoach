...
/**
 * @swagger
 * /api/v1/users/saved-workouts:
 *   get:
 *     tags:
 *       - Users
 *     summary: Get authenticated user's saved workouts
 *     description: Retrieve all workouts saved by the authenticated user with pagination and filters
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: query
 ...
 *                 message:
 *                   type: string
 *                   example: User saved workouts retrieved successfully
 *                 data:
 *                   type: object
 *                   properties:
 *                     savedWorkouts:
...
router.get('/saved-workouts',
  authenticate,
  asyncHandler(UserController.getSavedWorkouts)
);
